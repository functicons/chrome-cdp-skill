...
//
// Shared tabs are managed via the CDP Tab Control Chrome extension popup.
...
// ---------------------------------------------------------------------------
// Unix socket CDP client — talks to native-host.mjs
// ---------------------------------------------------------------------------
...
SETUP
  ...
  2. Run install.sh <extension-id> to set up native messaging
...
async function main() {
  ...
  if (cmd === 'list' || cmd === 'ls') {
    ...
    if (tabs.length === 0) {
      console.log('No shared tabs. Open the CDP Tab Control extension and share a tab.');
      return;
    }
